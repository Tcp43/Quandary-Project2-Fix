...
public class Interpreter {
    ...
    Object evaluate(Expr expr) {
        if (expr instanceof ConstExpr) {
            ...
            String name = ce.getFuncName();
            java.util.List<Expr> args = ce.getArgs();
        
            if ("main".equals(name)) {
                if (args.size() != 1) {
                    fatalError("main() expects 1 argument", EXIT_PARSING_ERROR);
                }
                long v = (long) evaluate(args.get(0));
                return callMain(v);
            }
        
            FunctionDecl fd = functions.get(name);
            if (fd == null) {
                fatalError("Undefined function: " + name, EXIT_PARSING_ERROR);
            }
            java.util.List<String> params = fd.getParamNames(); // or getParams()
            if (params.size() != args.size()) {
                fatalError("Arity mismatch in call to " + name, EXIT_PARSING_ERROR);
            }
            Map<String,Object> savedEnv = env;
            try {
                env = new HashMap<>();
                for (int i = 0; i < params.size(); i++) {
                    env.put(params.get(i), evaluate(args.get(i)));
                }
                Object ret = execute(fd.getBody());
                if (ret == null) {
                    fatalError("Missing return in function " + name, EXIT_PARSING_ERROR);
                }
                return ret;
            } finally {
                env = savedEnv;
